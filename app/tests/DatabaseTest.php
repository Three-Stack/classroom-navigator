<?php
/**
 * Tests the database connection to make sure it's working
 */
...
class DatabaseTest extends TestCase {
    ...
    public function testDB() {
        $db = DBConnection::db(); // Get the database connection
        
        // Create temp table
        $db->exec("CREATE TABLE IF NOT EXISTS `classes_test` (
        ...
        // Add some classes
        $db->exec("INSERT INTO `classes_test` (`class_id`, `name`, `time`) VALUES ('CS4800', 'Software Engineering', '8:30')");
        $db->exec("INSERT INTO `classes_test` (`class_id`, `name`, `time`) VALUES ('CS4650', 'Databases', '2:30')");
        $db->exec("INSERT INTO `classes_test` (`class_id`, `name`, `time`) VALUES ('CS4630', 'Undergraduate Seminar', '10:00')");
        $stmt = $db->prepare("SELECT * FROM `classes_test`");
        $stmt->execute();
    ...
    public function testCurl() {
        ...
        foreach($res as $r) {
            ...
            file_put_contents("/app/tmp_{$i}.txt", print_r($matches[1], true));
            $i++;
        }

        file_put_contents("/app/tmp.txt", print_r($res, true));
